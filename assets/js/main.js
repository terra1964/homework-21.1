    ...
    let frosts1 = 0;// кол-во заморозков (с + на -)
    for(i = 0; i < winterDays.length; i++){
        if(winterDays[i] >= 0 && winterDays[i + 1] < 0){
            frosts1 += 1;
        }
    }
        console.log(`Количество заморозков ${frosts1} дней`);

    let frost2 = 0;
    let maxday = 0; // период заморозков
    for (i = 0; i < winterDays.length; i++){
        if(winterDays[1] < 0){
            for (a = 0; a < winterDays.length; a++){
                if (winterDays[a] < 0){
                    frost2 += 1;
                if(frost2 > maxday){
                    maxday = frost2;
        ...
        console.log(`Максимальная продолжительность заморозков ${maxday} дней`);   
